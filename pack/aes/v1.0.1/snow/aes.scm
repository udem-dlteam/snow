...
;;; File: "aes.scm", Time-stamp: <2007-04-05 00:49:26 feeley>
...
(package* aes/v1.0.1
 (provide:
  ...
  (define* (aes-encrypt-subu8vector u8vect start end key (iv _)))
  (define* (aes-decrypt-subu8vector u8vect start end key (iv _)))
  (define* (aes-encrypt-u8vector u8vect key (iv _)))
  (define* (aes-decrypt-u8vector u8vect key (iv _))))
...
(define-macro (ref32-L u16vect i)
  `(snow-u16vector-ref ,u16vect (* ,i 2)))

(define-macro (ref32-H u16vect i)
  `(snow-u16vector-ref ,u16vect (+ 1 (* ,i 2))))

(define-macro (ref32 var-lo var-hi u16vect i body)
  `(let* ((,var-hi (* ,i 2))
          (,var-lo (snow-u16vector-ref ,u16vect ,var-hi))
          (,var-hi (snow-u16vector-ref ,u16vect (+ 1 ,var-hi))))
     ,body))

(define-macro (set32 lo hi u16vect i)
  `(let ((i (* ,i 2)))
     (snow-u16vector-set! ,u16vect i ,lo)
     (snow-u16vector-set! ,u16vect (+ 1 i) ,hi)))

(define-macro (xor32 dst-lo dst-hi x1-lo x1-hi x2-lo x2-hi body)
  `(let ((,dst-lo (snow-fxxor ,x1-lo ,x2-lo))
         (,dst-hi (snow-fxxor ,x1-hi ,x2-hi)))
     ,body))
...
(define-macro (get32 lo hi u8vect i body)
  `(let* ((index
           ,i)
          (,lo
           (u16 (snow-u8vector-ref ,u8vect (+ index 3))
                (snow-u8vector-ref ,u8vect (+ index 2))))
          (,hi
           (u16 (snow-u8vector-ref ,u8vect (+ index 1))
                (snow-u8vector-ref ,u8vect index))))
     ,body))
...
(define (build-table v f0 f1)
  (let ((t (snow-make-u16vector 512)))
    (let loop ((i 0))
      (if (< i 256)
          (let* ((i*2 (* i 2))
                 (x (vector-ref v i*2))
                 (y (vector-ref v (+ 1 i*2)))
                 (a (snow-fxarithmetic-shift-right x 8))
                 (b (snow-fxand #xff x))
                 (c (snow-fxarithmetic-shift-right y 8))
                 (d (snow-fxand #xff y)))
            (snow-u16vector-set! t i*2       (f1 a b c d))
            (snow-u16vector-set! t (+ 1 i*2) (f0 a b c d))
            (loop (+ i 1)))
          t))))
...
(define-macro (xorAA var-lo var-hi X-L X-H i Y-L Y-H body)
  `(let* ((,var-lo
           (snow-fxxor
            ...
            (snow-fxxor
             (ref32-L RCON ,i)
             (u16 (FSb-ref (snow-fxarithmetic-shift-right ,Y-H 8))
                  (FSb-ref (snow-fxand #xff ,Y-L))))))
          (,var-hi
           (snow-fxxor
            ,X-H
            (snow-fxxor
             (ref32-H RCON ,i)
             (u16 (FSb-ref (snow-fxarithmetic-shift-right ,Y-L 8))
                  (FSb-ref (snow-fxand #xff ,Y-H)))))))
     ,body))
...
(define-macro (xorBB var-lo var-hi X-L X-H Y-L Y-H body)
  `(let* ((,var-lo
           (snow-fxxor
            ,X-L
            (u16 (FSb-ref (snow-fxand #xff ,Y-L))
                 (FSb-ref (snow-fxarithmetic-shift-right ,Y-L 8)))))
          (,var-hi
           (snow-fxxor
            ,X-H
            (u16 (FSb-ref (snow-fxand #xff ,Y-H))
                 (FSb-ref (snow-fxarithmetic-shift-right ,Y-H 8))))))
     ,body))

(define (u8vector->aes-context key) ;; key: u8vector of length 16, 24 or 32
  (let ((len (snow-u8vector-length key)))
    (if (not (or (= len 16)
                 (= len 24)
                 (= len 32)))
        (snow-error "key must be a u8vector of length 16, 24 or 32")
        (let* ((keysize
                (* len 8))
               (nr
                (cond ((= keysize 256) 14)
          ...
          (let loop ((i 1))
            (if (< i nr)
                ...
                (let ((j (* i 4))
                  ...
                  (define (f offs)
                    (ref32 L H RK (+ k offs)
                    (let* ((i0 (snow-fxarithmetic-shift-right H 8))
                           (i1 (snow-fxand #xff H))
                           (i2 (snow-fxarithmetic-shift-right L 8))
                           (i3 (snow-fxand #xff L)))
...
(define-macro (FCOMB1 var-lo var-hi i Y0-H Y1-H Y2-L Y3-L body)
  `(let ((,var-lo
          (snow-fxxor
           (ref32-L RK ,i)
           (snow-fxxor
            (ref32-L FT0 (snow-fxarithmetic-shift-right ,Y0-H 8))
            (snow-fxxor
             (ref32-L FT1 (snow-fxand #xff ,Y1-H))
             (snow-fxxor
              (ref32-L FT2 (snow-fxarithmetic-shift-right ,Y2-L 8))
              (ref32-L FT3 (snow-fxand #xff ,Y3-L)))))))
         (,var-hi
          (snow-fxxor
           (ref32-H RK ,i)
           (snow-fxxor
            (ref32-H FT0 (snow-fxarithmetic-shift-right ,Y0-H 8))
            (snow-fxxor
             (ref32-H FT1 (snow-fxand #xff ,Y1-H))
             (snow-fxxor
              (ref32-H FT2 (snow-fxarithmetic-shift-right ,Y2-L 8))
              (ref32-H FT3 (snow-fxand #xff ,Y3-L)))))))
         (,i
          (+ ,i 1)))
     ,body))
...
(define-macro (FCOMB2 var-lo var-hi i Y0-H Y1-H Y2-L Y3-L body)
  `(let ((,var-lo
          (snow-fxxor
           (ref32-L RK ,i)
           (u16 (FSb-ref (snow-fxand #xff ,Y3-L))
                (FSb-ref (snow-fxarithmetic-shift-right ,Y2-L 8)))))
         (,var-hi
          (snow-fxxor
           (ref32-H RK ,i)
           (u16 (FSb-ref (snow-fxand #xff ,Y1-H))
                (FSb-ref (snow-fxarithmetic-shift-right ,Y0-H 8)))))
         (,i
          (+ ,i 1)))
     ,body))
...
(define-macro (RCOMB1 var-lo var-hi i Y0-H Y1-H Y2-L Y3-L body)
  `(let ((,var-lo
          (snow-fxxor
           (ref32-L RK ,i)
           (snow-fxxor
            (ref32-L RT0 (snow-fxarithmetic-shift-right ,Y0-H 8))
            (snow-fxxor
             (ref32-L RT1 (snow-fxand #xff ,Y1-H))
             (snow-fxxor
              (ref32-L RT2 (snow-fxarithmetic-shift-right ,Y2-L 8))
              (ref32-L RT3 (snow-fxand #xff ,Y3-L)))))))
         (,var-hi
          (snow-fxxor
           (ref32-H RK ,i)
           (snow-fxxor
            (ref32-H RT0 (snow-fxarithmetic-shift-right ,Y0-H 8))
            (snow-fxxor
             (ref32-H RT1 (snow-fxand #xff ,Y1-H))
             (snow-fxxor
              (ref32-H RT2 (snow-fxarithmetic-shift-right ,Y2-L 8))
              (ref32-H RT3 (snow-fxand #xff ,Y3-L)))))))
         (,i
          (+ ,i 1)))
     ,body))
...
(define-macro (RCOMB2 var-lo var-hi i Y0-H Y1-H Y2-L Y3-L body)
  `(let ((,var-lo
          (snow-fxxor
           (ref32-L RK ,i)
           (u16 (RSb-ref (snow-fxand #xff ,Y3-L))
                (RSb-ref (snow-fxarithmetic-shift-right ,Y2-L 8)))))
         (,var-hi
          (snow-fxxor
           (ref32-H RK ,i)
           (u16 (RSb-ref (snow-fxand #xff ,Y1-H))
                (RSb-ref (snow-fxarithmetic-shift-right ,Y0-H 8)))))
         (,i
          (+ ,i 1)))
     ,body))
            ...
            (if (>= k 0)
                (begin
                  (snow-u8vector-set!
                   ...
                   k
                   (snow-fxxor
                    (snow-u8vector-ref iv k)
                    (snow-u8vector-ref input (+ ipos k))))
                  (loop2 (- k 1)))))
            ...
            (if (>= k 0)
                (begin
                  (snow-u8vector-set!
                   ...
                   (snow-fxxor
                    (snow-u8vector-ref iv k)
                    (snow-u8vector-ref output (+ opos k))))
                  (loop2 (- k 1)))))
          (snow-subu8vector-move!
           input
           ipos
...
(define* (aes-encrypt-subu8vector
          ...
          key
          (iv (snow-make-u8vector block-size 0)))
...
(define* (aes-decrypt-subu8vector
  ...
  (let* ((context (u8vector->aes-context key))
    ...
    (if (not (= 0 (modulo len block-size)))
        (snow-error "length must be a multiple of 16")
...
(define* (aes-encrypt-u8vector
          ...
          key
          (iv (snow-make-u8vector block-size 0)))
  (aes-encrypt-subu8vector u8vect 0 (snow-u8vector-length u8vect) key iv))

(define* (aes-decrypt-u8vector
          u8vect
          key
          (iv (snow-make-u8vector block-size 0)))
  (aes-decrypt-subu8vector u8vect 0 (snow-u8vector-length u8vect) key iv))
