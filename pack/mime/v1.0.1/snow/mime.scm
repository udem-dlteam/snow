...
(define eol
  (snow-ISO-8859-1-string->u8vector (mime-eol-str)))
...
(define (fws? doc start end)
  (and start
       (let ((s (if (pair? start) (car start) start)))
         (let ((i (*wsp? doc s end)))
           (if (and (<= (+ i 3) end)
                    (= (snow-u8vector-ref doc i) (ch-return))
                    (= (snow-u8vector-ref doc (+ i 1)) (ch-linefeed))
                    (wsp? (snow-u8vector-ref doc (+ i 2))))
               (+ i 3)
               i)))))
...
(define (parse-quoted-string doc start end)
  (and start
       (let ((s (if (pair? start) (car start) start)))
         (if (and (< s end)
                  (= (snow-u8vector-ref doc s) (ch-doublequote)))
             (let loop ((i (+ s 1))
                        (rev-chars '()))
               (if (< i end)
                   (let ((next (snow-u8vector-ref doc i)))
...
(define (parse-symbol doc start end constituent?)
  (and start
       (let ((s (if (pair? start) (car start) start)))
         (if (< s end)
             (let ((first (snow-u8vector-ref doc s)))
               (if (constituent? first)
                   (let loop ((i (+ s 1))
                              (rev-chars (list (integer->char first))))
                     (cond ((and (< i end)
                                 (constituent? (snow-u8vector-ref doc i)))
                            =>
                            (lambda (next)
                              (loop (+ i 1)
                                    (cons (integer->char next) rev-chars))))
  ...
  (let ((first (parse-atom doc start end)))
    (and first
         (let loop ((i (car first))
                    (dot-atom (cdr first)))
           (if (and (< i end)
                    (= (snow-u8vector-ref doc i) (ch-dot)))
...
(define (parse-eol? doc start end)
  (and start
       (let ((s (if (pair? start) (car start) start)))
         (and (<= (+ s 2) end)
              (= (snow-u8vector-ref doc s) (ch-return))
              (= (snow-u8vector-ref doc (+ s 1)) (ch-linefeed))
              (+ s 2)))))
    ...
    (case type
      ...
      ((multipart)
       (let ((y (assq 'boundary (cddr x))))
         (if (not y)
             (snow-error "multipart boundary missing")
             (let ((parts
                                ...
                                (cond ((and disp
                                       ...
                                       (lambda (name-binding)
                                         (cons (string->symbol
                                               ...
                                               (cdr x))))
                                      (else
                                       (snow-error "form-data syntax error")))))
                            parts))
                      (else
                       parts)))))))
      ...
      (else
       ...
       (snow-error "unsupported content type")))))
...
(define (mime-encode doc)
  (cond ((string? doc)
        ...
        (else
         ...
         (snow-error "type cannot be encoded"))))

(define (mime-decode-text doc start end)
  (snow-subu8vector->ISO-8859-1-string doc start end))

(define (mime-encode-text doc)
  (cons '(content-type (text . plain))
        (snow-ISO-8859-1-string->u8vector doc)))

(define (mime-decode-application doc start end)
  (snow-subu8vector doc start end))
  ...
  (let ((boundary
    ...
    (define (search-boundary i prev-sop rev-parts)
      (let ((sol
             (let loop ((i (+ i 2)))
               (if (<= i end)
                   (if (and (= (snow-u8vector-ref doc (- i 2))
                               (ch-return))
                            (= (snow-u8vector-ref doc (- i 1))
                               (ch-linefeed)))
                       i
                       (loop (+ i 1)))
                   #f))))
    ...
    (define (start-of-line sol prev-sop rev-parts)
      (if (<= (+ sol (+ 4 (snow-u8vector-length boundary))) end)
          (let loop ((j 0))
            (if (< j (snow-u8vector-length boundary))
                (if (= (snow-u8vector-ref doc (+ sol j))
                       (snow-u8vector-ref boundary j))
                    (loop (+ j 1))
                    (search-boundary (+ sol j) prev-sop rev-parts))
                (let ((sop (+ sol (+ j 2))))
                  (let ((c1 (snow-u8vector-ref doc (- sop 2)))
                        (c2 (snow-u8vector-ref doc (- sop 1))))
                    (cond ((and (= c1 (ch-return))
                          ...
                          ((and (= c1 (ch-minus))
                                (= c2 (ch-minus))
                                (= (snow-u8vector-ref doc sop)
                                   (ch-return))
                                (= (snow-u8vector-ref doc (+ sop 1))
                                   (ch-linefeed)))
                           (cons (+ sop 2)
                                 (reverse
                                  (add-part (- sol 2) prev-sop rev-parts))))
...
(define (encode-multipart-with-boundary doc boundary-str)
  (let ((boundary
         (snow-ISO-8859-1-string->u8vector boundary-str)))
    (let loop ((lst doc)
               (rev-parts '()))
      (if (pair? lst)
          (let* ((x (car lst))
              ...
              (let ((x (assq field-name
                ...
                (if x
                    (let ((part ((caddr x) field)))
                      (loop (cdr lst)
                            (cons part rev-parts)))
                    (snow-error "unknown header field type")))
              (loop (cdr lst)
                    (cons (snow-ISO-8859-1-string->u8vector
                           (string-append
...
(define (mime-encode-x-www-form-urlencoded fields)
  ...
  (define (add-substring str start end lst)
    (cons (snow-ISO-8859-1-substring->u8vector str start end)
          lst))

  (define (add-urlencoded str lst)

    (define (nibble n)
      (string-ref "0123456789ABCDEF" (snow-fxand n 15)))
                  ...
                  (loop1 (+ j 1)
                         (add-substring
                          (let ((n (char->integer c)))
                            (string #\%
                                    (nibble (snow-fxarithmetic-shift-right n 4))
                                    (nibble n)))
                          0
                          3
...
(define (mime-string->content-type str)
  (let* ((doc (snow-ISO-8859-1-string->u8vector str))
         (len (snow-u8vector-length doc))
