...
;;; File: "base64.scm", Time-stamp: <2007-04-05 00:49:45 feeley>
...
(package* base64/v1.0.1
 (provide:

  (define (base64-substring->u8vector str start end))
  (define (base64-string->u8vector str))
  (define* (subu8vector->base64-string u8vect start end (width _)))
  (define* (u8vector->base64-string u8vect (width _))))
...
(define (fifo-next fifo)        (cdr fifo))
(define (fifo-next-set! fifo x) (set-cdr! fifo x))
(define (fifo-tail fifo)        (car fifo))
(define (fifo-tail-set! fifo x) (set-car! fifo x))
(define (fifo-elem fifo)        (car fifo))
(define (fifo-elem-set! fifo x) (set-car! fifo x))
...
(define (fifo->u8vector fifo start end)
  (snow-subu8vector
   (snow-apply-u8vector-append (fifo-next fifo))
   start
   end))

(define (u8vector-shrink u8vect len)
  (snow-subu8vector u8vect 0 len))

(define (fifo->string fifo start end)
  (substring
   (snow-apply-string-append (fifo-next fifo))
   start
   end))
...
(define (base64-substring->u8vector str start end)
  ...
  (define (err)
    (snow-error "base64 decoding error"))
  ...
  (define (wr-u8 x)
    (let ((ptr (vector-ref state 0)))
      ...
      (let ((fifo (vector-ref state 1))
            (i (snow-fxand ptr (- chunk-len 1))))
        (snow-u8vector-set!
         (if (= i 0)
             (let ((chunk (snow-make-u8vector chunk-len)))
  ...
  (define (add1 x0 x1)
    (add (+ (snow-fxarithmetic-shift-left x0 2)
            (snow-fxarithmetic-shift-right x1 4))))

  (define (add2 x0 x1 x2)
    (add1 x0 x1)
    (add (snow-fxand
          #xff
          (+ (snow-fxarithmetic-shift-left x1 4)
             (snow-fxarithmetic-shift-right x2 2)))))

  (define (add3 x0 x1 x2 x3)
    (add2 x0 x1 x2)
    (add (snow-fxand
          #xff
          (+ (snow-fxarithmetic-shift-left x2 6)
             x3))))
...
(define* (subu8vector->base64-string u8vect start end (width 0))
  ...
  (let loop ((i start)
    ...
    (if (<= (+ i 3) end)
        (let ((b0 (snow-u8vector-ref u8vect i))
              (b1 (snow-u8vector-ref u8vect (+ i 1)))
              (b2 (snow-u8vector-ref u8vect (+ i 2))))
          (let ((x0
                 (snow-fxarithmetic-shift-right b0 2))
                (x1
                 (snow-fxand
                  #x3f
                  (+ (snow-fxarithmetic-shift-left b0 4)
                     (snow-fxarithmetic-shift-right b1 4))))
                (x2
                 (snow-fxand
                  #x3f
                  (+ (snow-fxarithmetic-shift-left b1 2)
                     (snow-fxarithmetic-shift-right b2 6))))
                (x3
                 (snow-fxand #x3f b2)))
            (loop (+ i 3)
...
(define* (u8vector->base64-string u8vect (width 0))
  (subu8vector->base64-string u8vect 0 (snow-u8vector-length u8vect) width))
